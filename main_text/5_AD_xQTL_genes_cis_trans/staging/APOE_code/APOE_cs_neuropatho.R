...
indep_cs<-rbindlist(lapply(names(res_indep_cs$variant_id),function(n)data.table(cs_name=n,variant_id=res_indep_cs$variant_id[[n]],
                                                                                ...
                                                                                outcome=res_indep_cs$outcome[[n]])))
...
# cmd=paste('plink --bfile',tools::file_path_sans_ext(plink_file),
#           '--extract range',fp(out,'AD_snps_5pbwin.tsv'),
#           '--recode vcf',
#           '--out',fp(out,basename(plink_file)|>tools::file_path_sans_ext()|>paste0('.donors_apoecs.only')))
system(cmd)
#add snps infos to donors mtd
...
clin<-fread('../../../acandib/APOE_Ab/Metadata_Files/ROSMAP_clinical.csv')
bios<-fread('../../../acandib/APOE_Ab/Metadata_Files/ROSMAP_biospecimen_metadata.csv')
clinw<-merge(clin,bios[assay=='wholeGenomeSeq'][,.(individualID,specimenID)],by='individualID')
complete<-fread('../../../acandib/APOE_Ab/Metadata_Files/ROSMAP_xqtl_complete_samples_covariates_sex_death_pmi_study.csv')
...
unique(donors_apoecs$individualID)|>length()#OK

#add neuropatho
neuropath<-fread('main_text/5_AD_xQTL_genes_cis_trans/staging/APOE/dataset_707_basic_02-10-2022.n3711.clean.plink.n2409.28FEB2022.txt')
dim(neuropath)#266
...
#neuropatho analysis coreecting for APOE4 /2 gentoyep


donors_apoecs[,apoe4_dose:=str_count(apoe_genotype,'4')]
donors_apoecs[,apoe2_dose:=str_count(apoe_genotype,'2')]

donors_apoecs[,cogdx_binary:=ifelse(cogdx%in%c(4,5),'AD',ifelse(cogdx%in%1:3,'No AD',NA))]
donors_apoecs[,cogdx_merge:=ifelse(cogdx%in%c(4,5),'AD',ifelse(cogdx%in%2:3,'MCI',ifelse(cogdx==1,'NL',NA)))]
...
apoecs_clin<-rbindlist(lapply(covs_lin,function(c){
  message(c)
  if(!c%in%c('age_death','cts_mmse30_lv','cts_mmse30_lv_clinad','age_first_ad_dx_num','age_first_clinad_dx','cogng_path_slope')){
    donors_apoecs[,summary(lm(unlist(.SD)~dose+msex+educ+age_death+apoe4_dose+apoe2_dose))$coefficients|>data.table(keep.rownames = 'cov'),
                by='variant_id2',.SDcols=c][cov=='dose'][,cov:=c]
  }else if(c%in%c('cts_mmse30_lv','cts_mmse30_lv_clinad')){
...
apoecs_clogit<-rbindlist(lapply(covs_logis,function(c){
  message(c)
...
apoecs_clin[,padj:=p.adjust(pval,n = length(unique(indep_cs$cs_name))),by=c('cov','variant_id2')]
apoecs_clin[padj<0.05]


apoecs_clin[!cov%in%c('apoe4_dose','apoe2_dose')][padj<0.05]

apoecs_clin[padj<0.05][!cov%in%c('apoe4_dose','apoe2_dose')]$cs_id  |>unique()|>length()#3/19 

fwrite(apoecs_clin,fp(out,'res_lm_apoecs_adpathology_apoe4_2_correction.csv.gz'))


unique(apoecs_clin[padj<0.05][!cov%in%c('apoe4_dose','apoe2_dose')][order(cs_id,cov,padj)][,.(cs_id,cov,padj,gwas_source,outcome)],by=c('cs_id','cov'))

# 
#     cs_id                 cov        padj
#    <char>              <char>       <num>
# 1: set_27             caa_4gp 0.004708944
# 2: set_27            caa_neo4 0.007830511
# 3: set_27             tdp_st4 0.042915291
# 4: set_31 age_first_ad_dx_num 0.020960086
# 5: set_57        tangles_sqrt 0.003434581
#                                                                                                                 gwas_source
#                                                                                                                      <char>
# 1: AD_Kunkle_Stage1_2019; AD_Wightman_Full_2021; AD_Wightman_Excluding23andMe_2021; AD_Wightman_ExcludingUKBand23andME_2021
...
# 4:                                                                                                            AD_Bellenguez
# 5:                           AD_Bellenguez; AD_Kunkle_Stage1_2019; AD_Wightman_Full_2021; AD_Wightman_Excluding23andMe_2021
...
#APOE4 only
apoecs_clin2<-rbindlist(lapply(covs_lin,function(c){
  message(c)
  if(!c%in%c('age_death','cts_mmse30_lv','cts_mmse30_lv_clinad','age_first_ad_dx_num','age_first_clinad_dx','cogng_path_slope')){
...
apoecs_clogit2<-rbindlist(lapply(covs_logis,function(c){
  message(c)
...
apoecs_clin2[,padj:=p.adjust(pval,n = length(unique(indep_cs$cs_name))),by=c('cov','variant_id2')]
apoecs_clin2[padj<0.05]


apoecs_clin2[!cov%in%c('apoe4_dose','apoe2_dose')][padj<0.05]

apoecs_clin2[padj<0.05][!cov%in%c('apoe4_dose','apoe2_dose')]$cs_id  |>unique()|>length()#4/19 

fwrite(apoecs_clin2,fp(out,'res_lm_apoecs_adpathology_apoe4_correction.csv.gz'))


unique(apoecs_clin2[padj<0.05][!cov%in%c('apoe4_dose','apoe2_dose')][order(cs_id,cov,padj)][,.(cs_id,cov,variant_id2,padj,gwas_source,outcome)],by=c('cs_id','cov'))
# cs_id                 cov              variant_id2        padj
# <char>              <char>                   <char>       <num>
